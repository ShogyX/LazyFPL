...
import { motionScale } from "./ui";

// x values may be strings (labels); series values are numbers. Kept loose so
// callers can pass either without ceremony — this is a low-level primitive.
type Row = Record<string, any>; // eslint-disable-line @typescript-eslint/no-explicit-any
export interface Series { key: string; label: string; color?: string; axis?: "left" | "right"; width?: number; dashed?: boolean }
...
export function niceTicks(min: number, max: number, n = 4): number[] {
  ...
  const out: number[] = []; for (let v = lo; v <= hi + 1e-9; v += step) out.push(+v.toFixed(6));
  return out;
}

const fmtId = (v: number) => String(v);

export function LineChart({ data, xKey, series, height = 280, area, yDomain, rightDomain,
  yFormat = fmtId, rightFormat = fmtId, xFormat = fmtId, legend = true }: {
  data: Row[]; xKey: string; series: Series[]; height?: number; area?: boolean;
  yDomain?: [number, number]; rightDomain?: [number, number];
  yFormat?: (v: number) => string; rightFormat?: (v: number) => string; xFormat?: (v: number) => string; legend?: boolean;
}) {
  const [wrapRef, W] = useMeasure();
  const [hi, setHi] = useState<number | null>(null);
  const motion = motionScale();
  const hasRight = series.some((s) => s.axis === "right");
  const padL = 38, padR = hasRight ? 38 : 12, padT = 12, padB = 26;
  ...
  const extent = (keys: Series[], dom?: [number, number]): [number, number] => {
    if (dom) return dom;
    let mn = Infinity, mx = -Infinity;
    data.forEach((d) => keys.forEach((s) => { const v = d[s.key]; if (v != null) { mn = Math.min(mn, v); mx = Math.max(mx, v); } }));
    if (!isFinite(mn)) { mn = 0; mx = 1; }
    const pad = (mx - mn) * 0.12 || 1; return [mn - pad * 0.3, mx + pad];
  };
  ...
  const X = (i: number) => padL + (xN <= 1 ? iw / 2 : (i / (xN - 1)) * iw);
  const YL = (v: number) => padT + ih - ((v - lMin) / (lMax - lMin || 1)) * ih;
  const YR = (v: number) => padT + ih - ((v - rMin) / (rMax - rMin || 1)) * ih;
  ...
  const onMove = (e: React.MouseEvent) => {
    ...
    setHi(Math.max(0, Math.min(xN - 1, Math.round(((x - padL) / iw) * (xN - 1)))));
  };
  const every = Math.ceil(xN / Math.max(4, Math.floor(iw / 52)));
  const showX = (i: number) => i === xN - 1 || (i % every === 0 && (xN - 1 - i) >= every * 0.6);
    ...
    <div ref={wrapRef} style={{ position: "relative", width: "100%" }}>
      {W > 0 && (
        <svg width={W} height={H} style={{ display: "block", overflow: "visible" }}>
          {ticks.map((t, k) => (
            <g key={k}>
              <line x1={padL} x2={padL + iw} y1={YL(t)} y2={YL(t)} stroke="var(--line)" />
          ...
          {area && leftKeys[0] && (
            <path d={`${path(leftKeys[0])} L${X(xN - 1)} ${padT + ih} L${X(0)} ${padT + ih} Z`} fill={leftKeys[0].color || SERIES[0]} opacity="0.10" />
          )}
          {hi != null && <line x1={X(hi)} x2={X(hi)} y1={padT} y2={padT + ih} stroke="var(--line-2)" strokeDasharray="3 3" />}
          {series.map((s, si) => {
            const len = 1400;
            return <path key={s.key} d={path(s)} fill="none" stroke={s.color || SERIES[si % 6]} strokeWidth={s.width || 2.4}
              strokeLinecap="round" strokeLinejoin="round" strokeDasharray={s.dashed ? "5 5" : len} strokeDashoffset={s.dashed ? 0 : len}
              style={s.dashed ? undefined : { animation: `dashIn ${1.05 / motion}s cubic-bezier(.3,.8,.3,1) ${si * 0.08}s forwards` }} />;
          })}
      ...
      {hi != null && W > 0 && (
        <div style={{ position: "absolute", top: 4, left: Math.min(W - 150, Math.max(0, X(hi) + 10)), background: "var(--surface-3)", border: "1px solid var(--line-2)", borderRadius: 9, padding: "8px 10px", pointerEvents: "none", boxShadow: "var(--shadow)", minWidth: 110, zIndex: 5 }}>
          ...
          {series.map((s, si) => data[hi][s.key] != null ? (
            <div key={s.key} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 12, marginTop: 2 }}>
              ...
              <span className="num" style={{ marginLeft: "auto", fontWeight: 700 }}>{(s.axis === "right" ? rightFormat : yFormat)(data[hi][s.key])}</span>
            </div>
          ) : null)}
        </div>
      )}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 16px", marginTop: 8, paddingLeft: padL }}>
          {series.map((s, si) => (
            <span key={s.key} style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 12, color: "var(--fg-dim)" }}>
              <span style={{ width: 14, height: 3, borderRadius: 2, background: s.color || SERIES[si % 6], opacity: s.dashed ? 0.6 : 1 }} />{s.label}
            </span>
          ))}
        </div>
      )}
...
export function BarChart({ data, xKey, series, height = 280, yFormat = fmtId, xFormat = fmtId, legend = true, horizontalLabels }: {
  data: Row[]; xKey: string; series: Series[]; height?: number;
  yFormat?: (v: number) => string; xFormat?: (v: number) => string; legend?: boolean; horizontalLabels?: boolean;
}) {
  const [wrapRef, W] = useMeasure();
  const [hi, setHi] = useState<number | null>(null);
  const [grown, setGrown] = useState(false);
  const motion = motionScale();
  useEffect(() => { const id = setTimeout(() => setGrown(true), 30); return () => clearTimeout(id); }, []);
  const padL = 38, padR = 10, padT = 12, padB = horizontalLabels ? 44 : 26;
  const H = height, iw = Math.max(10, W - padL - padR), ih = H - padT - padB;
  let mx = 0; data.forEach((d) => series.forEach((s) => { mx = Math.max(mx, d[s.key] || 0); }));
  const ticks = niceTicks(0, mx, 4); mx = ticks[ticks.length - 1];
  const Y = (v: number) => padT + ih - (v / (mx || 1)) * ih;
  const groupW = iw / Math.max(1, data.length), barGap = 0.22 * groupW, innerW = groupW - barGap;
  const bw = innerW / series.length;
  const lblEvery = horizontalLabels ? 1 : Math.max(1, Math.ceil(data.length / Math.max(6, Math.floor(iw / 34))));
  return (
    <div ref={wrapRef} style={{ position: "relative", width: "100%" }}>
      {W > 0 && (
        <svg width={W} height={H} style={{ display: "block", overflow: "visible" }}>
            ...
            return (
              <g key={i} onMouseEnter={() => setHi(i)} onMouseLeave={() => setHi(null)}>
                ...
                {series.map((s, si) => {
                  ...
                  return <rect key={s.key} x={gx + si * bw + bw * 0.12} width={bw * 0.76} y={padT + ih - h} height={Math.max(0, h)}
...
export function Sparkline({ data, width = 110, height = 30, color = "var(--accent)", fill = true }:
  ...
  const Y = (v: number) => height - 3 - ((v - mn) / (mx - mn || 1)) * (height - 6);
